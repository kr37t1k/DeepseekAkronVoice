#!/usr/bin/env python3
"""
Enhanced LLaMA Local Server with Streaming Support for LM Studio
This server can run on your computer and provide AI responses via API with document parsing support and streaming output
"""
...
class MockLlamaModel:
    ...
    def create_chat_completion(self, messages, temperature=0.7, max_tokens=500, stream=False, **kwargs):
        ...
        if stream:
            ...
            return self._generate_streaming_response(response)
        else:
            return {
                "choices": [{
                    "message": {
    ...
    def _generate_streaming_response(self, response):
...
class EnhancedLlamaRequestHandler(BaseHTTPRequestHandler):
                ...
                "data": [
                    {
                        ...
                        "created": int(time.time()),
                        "owned_by": "local",
                        "capabilities": ["chat", "document_parsing", "streaming"]
                    }
                ]
            }
    ...
    def handle_chat_completions(self):
        ...
        try:
            ...
            if stream:
                self._handle_streaming_request(messages, temperature, max_tokens)
            else:
                self._handle_non_streaming_request(messages, temperature, max_tokens)
    ...
    def _handle_non_streaming_request(self, messages, temperature, max_tokens):
        ...
        try:
            ...
            logger.info(f"Generated non-streaming response for request with {len(messages)} messages")
        except Exception as e:
            logger.error(f"Error in non-streaming request: {e}")
            try:
                self.send_error(500, f"Non-streaming request error: {str(e)}")
            except:
                logger.warning("Could not send error response for non-streaming request")

    def _handle_streaming_request(self, messages, temperature, max_tokens):
        ...
        try:
            ...
            completion_generator = self.model.create_chat_completion(
                messages=messages,
                temperature=temperature,
                max_tokens=max_tokens,
                stream=True
            ...
            for chunk in completion_generator:
                ...
                try:
                    ...
                    logger.warning(f"Error writing streaming chunk: {write_error}")
                    break

            # Send the [DONE] marker to indicate end of stream
            done_chunk = "data: [DONE]\n\n"
            try:
                self.wfile.write(done_chunk.encode('utf-8'))
                self.wfile.flush()
            except (BrokenPipeError, ConnectionResetError, socket.error) as write_error:
                logger.warning(f"Error writing [DONE] marker: {write_error}")
            logger.info(f"Completed streaming response for request with {len(messages)} messages")
        except Exception as e:
            ...
            try:
                ...
                chunk_data = json.dumps(error_chunk, separators=(',', ':'))
                sse_chunk = f"data: {chunk_data}\n\n"
                self.wfile.write(sse_chunk.encode('utf-8'))
                self.wfile.flush()
            except:
                logger.warning("Could not send error response for streaming request")
...
def run_server(model, host='0.0.0.0', port=8001):
    ...
    logger.info("Server supports streaming responses for LM Studio compatibility")
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced LLaMA Local Server with document parsing and streaming')
    ...
    parser.add_argument('--test', action='store_true',
                       help='Run with mock model for testing without actual model files')
    
    args = parser.parse_args()
    
    # Load the model
    if LLAMA_AVAILABLE and not args.test:
        if not args.model_path:
            logger.error("--model-path is required when not using --test mode")
            return
        ...
        model = MockLlamaModel()
        logger.info("Using mock model for testing (either --test flag used or llama-cpp-python not available)")
